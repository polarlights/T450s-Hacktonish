 ...
 * 
 * Disassembly of DSDT.aml, Mon Nov 16 13:45:47 2015
 *
 ...
 */
DefinitionBlock ("DSDT.aml", "DSDT", 1, "LENOVO", "TP-JB   ", 0x00001180)
{
    ...
    External (_SB_.PCI0.LPC_.TPM_, UnknownObj)
    External (_SB_.PCI0.LPC_.TPM_._STA, MethodObj)    // 0 Arguments
    External (_SB_.PCI0.LPC_.TPM_.BUF1, BuffObj)
    External (_SB_.PCI0.LPC_.TPM_.CMOR, MethodObj)    // 0 Arguments
    ...
    IRQNoFlags() { 0, 8, 11, 15 }
                ...
                Device (EC)
                {
                    ...
                    Method (P6XH, 2, NotSerialized)
                    {
                        ...
                        If (LEqual (Arg0, 0x01))
                        {
                            Store (Or (And (P62D, 0xFFFF00FF), ShiftLeft (Arg1, 0x08)
                                ), P62D) /* \P62D */
                        }

                        If (LEqual (Arg0, 0x02))
                        {
                            Store (Or (And (P62D, 0xFF00FFFF), ShiftLeft (Arg1, 0x10)
                                ), P62D) /* \P62D */
                        }

                        If (LEqual (Arg0, 0x03))
                        {
                            Store (Or (And (P62D, 0x00FFFFFF), ShiftLeft (Arg1, 0x18)
                                ), P62D) /* \P62D */
                        }
            ...
            Device (XHCI)
            {
                ...
                Method (_PS0, 0, Serialized)  // _PS0: Power State 0
                {
                    ...
                    If (LAnd (LEqual (PCHS, 0x02), LEqual (PCHG, 0x01)))
                    {
                        ...
                        {
                            Store (0x00, Local7)
                            While (LEqual (LAnd (LAnd (LAnd (PSC1, 0x00080000), LAnd (PSC2, 0x00080000)), 
                                LAnd (LAnd (PSC3, 0x00080000), LAnd (PSC4, 0x00080000))), 0x00))
                            {
                                Stall (0x0A)
                                Increment (Local7)
        ...
        If (Local0)
        {
            ...
            If (LEqual (Arg0, 0x03))
            {
                ...
                If (And (Local1, 0x20))
                {
                    ...
                    {
                        If (LOr (LAnd (\_SB.PCI0.LPC.EC.HB0A, LGreater (And (\_SB.PCI0.LPC.EC.HB0S, 0x07), 0x01)), 
                            LAnd (\_SB.PCI0.LPC.EC.HB1A, LGreater (And (\_SB.PCI0.LPC.EC.HB1S, 0x07), 0x01))))
                        {
                            Store (0x00, \FFCL)
                        }
                        Else
